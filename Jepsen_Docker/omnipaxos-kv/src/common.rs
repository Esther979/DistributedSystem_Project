...
pub mod kv {
    ...
    pub enum KVCommand {
        ...
        Delete(String),
        Get(String),
        Cas(String, String, String),  // ← (key, expected_value, new_value)
    }
        ...
        fn create(entries: &[Command]) -> Self {
            ...
            for e in entries {
                match &e.kv_cmd {
                    ...
                    KVCommand::Get(_) => {}   // 读操作不影响 snapshot
                    KVCommand::Cas(key, _expected, new_value) => {
                        // snapshot 里直接记录结果值（snapshot 时 CAS 已经被 commit，说明已成功）
